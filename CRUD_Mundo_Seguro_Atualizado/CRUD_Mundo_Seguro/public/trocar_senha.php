...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!$errors){$s=db()->prepare('SELECT senha FROM usuarios WHERE username=?');$s->execute([current_user()['username']]);$hash=$s->fetchColumn();if(!$hash||!password_verify($atual,(string)$hash))$errors[]='Senha atual incorreta';else{$u=db()->prepare('UPDATE usuarios SET senha=?,primeiro_acesso=0,dt_acesso=NOW() WHERE username=?');$u->execute([password_hash($nova,PASSWORD_DEFAULT),current_user()['username']]);$_SESSION['user']['primeiro_acesso']=0;audit('Senha alterada com sucesso.');flash('success','Senha alterada com sucesso.');redirect('dashboard.php');}}
}
